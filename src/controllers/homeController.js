...
const cache = {}; // Basit bir bellek önbelleği, daha gelişmiş çözümler kullanabilirsiniz.
...
const homeShow = async (req, res, next) => {
    try {
        ...
        // Önbellek kontrolü
        const cacheKey = `homeShow_${token}`;
        if (cache[cacheKey]) {
            // Önbellekte varsa, önbellekten al ve işlemleri sonlandır
            return res.render('user/anasayfa', cache[cacheKey]);
        }

        // Veritabanı sorgularını paralel olarak yürütme
        const [myacc, banner, productList, encokoyalanurunler, aranan, Product_WishListCounter] = await Promise.all([
            User.find({ usertoken: token }),
            Urun.find({ banner: '1' }).sort({ timestamp: -1 }).limit(10).allowDiskUse(true),
            Urun.find({ active: '1' }).sort({ timestamp: -1 }).skip(Math.random() * 7000).limit(50).allowDiskUse(true),
            Urun.find({ urun_oysayisi: { $gt: 0 } }).sort({ timestamp: -1 }).limit(25).allowDiskUse(true),
            Urun.find({ tiklanmasayisi: { $gt: 1 } }).sort({ timestamp: -1 }).limit(25).allowDiskUse(true),
            Urun.find({ Product_WishListCounter: { $gt: 0 } }).sort({ timestamp: -1 }).limit(16).allowDiskUse(true),
        ]);

        // Encokarananurunler dizisinin oluşturulması
        const encokarananurunler = aranan.slice(-21).reverse();

        // Urunler dizilerinin oluşturulması
        const urunler = productList.slice(0, 6);
        const urunler2 = productList.slice(0, 4);
        const urunler3 = productList.slice(4, 8);
        const urunler4 = productList.slice(8, 12);
        const urunler5 = productList.slice(0, 12);

        // JWT doğrulama
        jwt.verify(token, jwtSecretKey, (e, decoded) => {
            if (decoded) {
                const data = {
                    ...
                    encokoyalanurunler,
                    encokarananurunler,
                    Product_WishListCounter
                };

                // Önbelleğe kaydet
                cache[cacheKey] = data;

                res.render('user/anasayfa', data);
            } else {
                const myacc = false;
                const data = {
                    layout: '../layouts/mainSecond_Layout',
                    title: `Yapx | Anasayfa`,
                    description: ``,
                    keywords: ``,
                    banner,
            ...
            const verified = jwt.verify(token, jwtSecretKey, async (e, decoded) => {
                if (decoded) {
                    const productList = await User.find({ usertoken: token }).then(Product => {
                        res.render('user/BireyselHesapAyarlari', { layout: '../layouts/logged', title: `Yapx | Hakkımızda`, description: ``, keywords: ``, Product, myacc })
                    })
...
const changePassword = async (req,res,next) => {
    try{
        ...
        const isMatch = await bcrypt.compare(req.body.guncelsifre, findUser.sifre || findUser.Password);
        if(isMatch){
            const Informations = {
...
const mesajgonder = async (req, res, next) => {
    try {
        ...
        const [sender, receiver] = await Promise.all([
            ...
            User.findOne({ Meslek_URL: userid }).then(user => user || User.findOne({ Dukkan_UniqueID: userid })),
        ]);
    ...
    try {
        ...
        if ((req.body.Password == req.body.rePassword) && KullaniciSorgu == 0) {
            const data = {
                ...
                Meslek_Photos: ["yapxmeslek_default.png"]
            }
            const yenikullanici = new User(
                data
            );
...
const bireyseluyeliksozlesmesi = async (req,res,next) => {
    try {
        ...
        const verified = jwt.verify(token, jwtSecretKey, async (e, decoded) => {
            if (decoded) {
                res.render('user/sozlesmeler/BireyselUyelikSozlesmesi', { layout: '../layouts/logged', title: `Yapx | Sozlesmeler`, description: ``, keywords: `` , myacc})
            } else {
                res.render('user/sozlesmeler/BireyselUyelikSozlesmesi', { layout: '../layouts/mainSecond_Layout', title: `Yapx | Sozlesmeler`, description: ``, keywords: ``,myacc })
            }
        })
...
const kurumsaluyeliksozlesmesi = async (req,res,next) => {
    try {
        ...
        const verified = jwt.verify(token, jwtSecretKey, async (e, decoded) => {
            if (decoded) {
                res.render('user/sozlesmeler/KurumsalUyelikSozlesmesi', { layout: '../layouts/logged', title: `Yapx | Sozlesmeler`, description: ``, keywords: `` , myacc})
            } else {
                res.render('user/sozlesmeler/KurumsalUyelikSozlesmesi', { layout: '../layouts/mainSecond_Layout', title: `Yapx | Sozlesmeler`, description: ``, keywords: ``,myacc })
            }
        })
...
const mesafelisatissozlesmesi = async (req,res,next) => {
    try {
        ...
        const verified = jwt.verify(token, jwtSecretKey, async (e, decoded) => {
            if (decoded) {
                res.render('user/sozlesmeler/MesafeliSatisSozlesmesi', { layout: '../layouts/logged', title: `Yapx | Sozlesmeler`, description: ``, keywords: `` , myacc})
            } else {
                res.render('user/sozlesmeler/MesafeliSatisSozlesmesi', { layout: '../layouts/mainSecond_Layout', title: `Yapx | Sozlesmeler`, description: ``, keywords: ``,myacc })
            }
        })
...
const hakkimizda = async (req,res,next) => {
    try {
        ...
        const verified = jwt.verify(token, jwtSecretKey, async (e, decoded) => {
            if (decoded) {
                res.render('user/sozlesmeler/Hakkimizda', { layout: '../layouts/logged', title: `Yapx | Sozlesmeler`, description: ``, keywords: `` , myacc})
            } else {
                res.render('user/sozlesmeler/Hakkimizda', { layout: '../layouts/mainSecond_Layout', title: `Yapx | Sozlesmeler`, description: ``, keywords: ``,myacc })
            }
        })
...
const updateVisibility = async (req, res) => {
    try {
        const { Id_Visibility, Date_Visibility, Shop_Name_Visibility, Address_Visibility,Name_Visibility } = req.body;
        const usertoken = req.cookies.usertoken;

        // Kullanıcının yetkilendirilip yetkilendirilmediğini kontrol etmek için usertoken kullanılır.
        const findUser = await User.findOne({ usertoken });

        if (!findUser) {
            return res.status(404).json({ error: 'User not found' });
        }

        // Kullanıcı görünürlük işlemleri
        // Örneğin, sadece belirli bir kullanıcı türü bu işlemi gerçekleştirebilirse:
        if (findUser.userid === "5" || findUser.userid === "3") {
            // İlgili alanları güncelle
            await User.findByIdAndUpdate(findUser._id, {
                Id_Visibility: Id_Visibility === "True",
                Date_Visibility: Date_Visibility === "True",
                Shop_Name_Visibility: Shop_Name_Visibility === "True",
                Address_Visibility: Address_Visibility === "True",
                Name_Visibility: Name_Visibility === "True",
            });
...
module.exports = {
    ...
    kayitbasarili,
    meslekkayitbasarili,
    bireyseluyeliksozlesmesi,
    kurumsaluyeliksozlesmesi,
